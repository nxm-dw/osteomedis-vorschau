...
(function(){
var z = $$(".c-zahl"); if (!z.length) return;
function zaehle(el){
var ziel = parseInt(el.dataset.ziel, 10) || 0;
if (RM || ziel === 0){ el.textContent = String(ziel); return; }
var start = Date.now(), dauer = 1200;
var iv = setInterval(function(){
var p = Math.min(1, (Date.now()-start)/dauer);
var e = 1 - Math.pow(1-p, 3);
el.textContent = String(Math.round(ziel*e));
if (p >= 1) clearInterval(iv);
}, 16);
}
if (!("IntersectionObserver" in window)){ z.forEach(function(el){ el.textContent = el.dataset.ziel; }); return; }
var io = new IntersectionObserver(function(es){
es.forEach(function(en){ if (en.isIntersecting){ zaehle(en.target); io.unobserve(en.target); } });
}, {threshold:.4});
z.forEach(function(el){ io.observe(el); });
})();
...
(function(){
var b = $("#cStimmen"); if (!b) return;
var q = $$("blockquote", b), p = $$(".c-stimmen-punkte button"), i = 0;
function zeige(n){
i = (n + q.length) % q.length;
q.forEach(function(el, k){ el.classList.toggle("is-on", k === i); });
p.forEach(function(el, k){ el.setAttribute("aria-pressed", String(k === i)); });
}
$$(".c-stimmen-pfeil").forEach(function(el){
el.addEventListener("click", function(){ zeige(i + parseInt(el.dataset.richtung, 10)); });
});
p.forEach(function(el, k){ el.addEventListener("click", function(){ zeige(k); }); });
})();
})();